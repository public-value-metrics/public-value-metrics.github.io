...
tenancy — that let a practitioner monetise an outcome without commissioning bespoke valuation
...
[Social Value Act](../social-value-act/) bid evaluations: without them, rigorous monetisation would
be affordable only to organisations large enough to commission their own studies.
...
catalogue" — a table mapping each outcome a product or service can plausibly claim to a named proxy,
its source database, its publication date, and a version identifier — so that different teams
across an organisation do not each pick slightly different values for the same outcome. Wrapping
...
- **Treating proxies as precise.** Most published proxies are modelled averages from wellbeing
