...
GiveWell's chain for a bednet program, illustratively:
...
observed price; it is a modeled estimate built from several separately uncertain inputs.
...
- **Bednets (AMF-style)**: roughly $4,500 per life saved on GiveWell's published worked example
  ...
  rate and year used.
- **Deworming program**: no plausible mortality benefit at all, but strong evidence of long-run
...
the same question — what is the best return per pound — in a monetized-value idiom rather than a
lives/DALYs idiom.
...
- **Treating a cost-effectiveness estimate as a fixed price.** It is a model output with several
  ...
  date and version.
- **Ignoring funging/displacement.** Funding an organization that would have received the money
