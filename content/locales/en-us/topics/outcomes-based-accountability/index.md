...
the population doing well?" (population accountability) and "is this specific program doing
well?" (performance accountability). Conflating the two is, in Friedman's telling, the single most
common reason well-run programs get blamed for population trends they never had the power to
move.
...
program-level activity counts (clients seen, referrals made) that say nothing about whether
...
measures (how well a specific program serves its specific clients) belong to one agency and
...
commissioning. The practical stakes are contractual: a housing program should not be defunded
...
Population accountability (the "big picture" a community, region, or nation shares):
  ...
  → no single program owns the indicator; movement requires many contributors

Performance accountability (what one program is responsible for):
  How much did we do?      — activity volume (clients served, units delivered)
  How well did we do it?   — quality/efficiency (% completing program, cost per client)
  Is anyone better off?    — the outcome that matters (% in employment 6 months after
                              program, before/after or against a comparison group)

A program is judged on the third performance question, never on the population
indicator directly, unless its scale and design could plausibly move it alone.
```

## Worked example

**City-funded employment support program**, 500 participants/year, contracted by a local
authority under an RBA-style performance framework:

```
Population indicator (context, not the program's scorecard):
  City unemployment rate: 6.2% (up from 5.8% the prior year, driven by a factory closure
  outside the program's control)

Performance measures (the program's actual accountability):
...
Under a population-accountability read, the program looks like it is failing — the city's
unemployment rate went up on its watch. Under RBA's performance-accountability read, the program
...
reasons (a factory closure) entirely outside the program's control.
...
- **Paying or penalizing a program against a population indicator it cannot control**: this is
  the single mistake RBA exists to prevent; always trace whether the program is a major or minor
...
- **No comparison group for "better off"**: a before/after change with no counterfactual conflates
  program effect with the trend the population would have shown anyway.
