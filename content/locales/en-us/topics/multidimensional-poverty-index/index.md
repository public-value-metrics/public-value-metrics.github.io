...
UNDP now covers over 110 countries. For software built for anti-poverty programs — cash
transfers, social care triage, aid targeting — the MPI's indicator set is often the closest thing
to a standardized deprivation schema already validated across dozens of national statistical
offices.
...
- Case-management and eligibility systems for social programs often already store several of
...
- MPI-style indicator dashboards pair naturally with [cost per beneficiary](../cost-per-beneficiary/)
  reporting for anti-poverty programs: cost per point of MPI reduction is a defensible unit for
...
- **Assuming MPI-poor and income-poor are the same population** — OPHI's own country briefs
  typically show only partial overlap between the two; a program targeting only the income-poor
