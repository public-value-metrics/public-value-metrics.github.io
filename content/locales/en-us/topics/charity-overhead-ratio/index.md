...
thoroughly discredited efficiency metrics in the sector, with the organizations that popularized
...
rather than "program" cost. A charity that starves its back office to report 5% overhead may be
...
Common variants:
  Program ratio        = Program (direct charitable) spend / Total expenditure
                          = 1 − overhead ratio
  Fundraising efficiency  = Fundraising cost / Funds raised
```

None of these formulas contains any information about outcomes achieved. A charity can minimize
...
Finance and grant-reporting software for the sector often hard-codes the overhead/program split
...
- **Using overhead ratio as a screening cut-off.** Rejecting any charity above an arbitrary
  ...
  well-evaluated organizations and rewards underinvestment.
- **Miscategorizing direct-delivery cost as overhead**, or vice versa — accounting conventions for
  what counts as "program" versus "admin" vary enough between charities that ratios are often
