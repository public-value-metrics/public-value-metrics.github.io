...
treatments using cost per Quality-Adjusted Life Year — but applied to non-health public programs:
education interventions per pupil-outcome-point, housing programs per household prevented from
homelessness, employment programs per sustained job outcome.
...
CEA cannot, by itself, say whether a program is worth funding at all — only which of several
...
strategic judgement outside the math. Where outcomes genuinely cannot be reduced to one unit —
because a program produces several outcomes that matter in different ways — use
...
**National government**: a literacy catch-up program is compared across three delivery models on
...
- **Choosing an outcome unit that is really an output.** "Referrals made" or "sessions delivered"
  measure activity, not the outcome the program exists to produce; CEA on outputs produces a
...
- What Works Center for Homelessness Impact. Cost-effectiveness evidence on homelessness
