...
```
Lead time      → weeks in the pipeline × CoD, see cost-of-delay-in-public-programs
Failure rate   → citizen-facing incident rate: CFR × cost per redirected
                 contact-center call (or per failed statutory transaction)
...
Reliability    → benefit discount: a service at 99% availability delivers
                 ≈ 0.99 of its modeled benefit — the delivery analog
...
- **Optimizing one metric alone**: speed without change failure rate is the classic throughput-instability trade-off — report all four together, not as a single score.
