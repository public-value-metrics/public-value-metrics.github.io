...
charity would deliver the same program with or without a particular £10,000 gift — because it
...
in program evaluation: value created is only creditable to a funder to the extent it would not
...
- **Charity C**: has a fully funded core program with £2 million in reserves and a waiting list
...
- **Charity D**: a small, evidence-backed program that has publicly stated it will have to turn
...
- **Assuming linear cost-effectiveness at scale.** The cheapest-to-reach beneficiaries are often
  served first; marginal cost per outcome frequently rises as a program expands, so the ROI on
