...
change which of two competing programs shows the higher net present value, particularly when
...
**Two competing local programs**, each with an unweighted net benefit of £2 million/year, competing
for the same regional growth fund:

- *Program A*: a business-support scheme in a prosperous town, average household income £45,000
  (roughly 1.3× the assumed national average of £35,000).
- *Program B*: a skills program in a deprived ward, average household income £18,000 (roughly
...
Unweighted, the two programs are tied. Weighted for distributional impact, Program B's benefit is
...
- When an algorithm allocates a scarce resource (appointment slots, caseworker time, a subsidy), an
  unweighted "maximize total benefit" objective function will, by construction, reproduce the same
  bias the Green Book's weighting exists to correct — flag this explicitly to policy owners before
  optimizing.

## Pitfalls

- **Applying distributional weights inconsistently across a portfolio.** Weighting one program's
