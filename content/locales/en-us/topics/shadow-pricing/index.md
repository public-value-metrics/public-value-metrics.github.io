...
time, unemployed labor — published centrally so every department uses the same number.
...
unemployed labor — have no market price at all, or a market price that misrepresents their true
...
The shadow wage rate follows a similar logic on the labor side. Employing someone who would
...
society's resources — so Green Book guidance sets a shadow price below the market wage for labor
drawn from unemployment, reflecting the true opportunity cost of that labor (see
...
Shadow wage rate (SWR):
  ...
  Typically expressed as a fraction of the market wage (e.g. SWR = 0.6
    ...
    guidance on labor markets with spare capacity)
```
...
**National government**: a flood defense scheme appraisal estimates it avoids 400 tonnes of CO2e
...
benefits and distort the ranking against alternative flood defense designs with different carbon
profiles.

**Local authority**: a council's employment support program for long-term unemployed residents
places 150 people into jobs paying £11/hour. Valuing this using the full market wage would credit
the program with £11 × hours worked as a social benefit, but the shadow wage rate approach
recognizes that these were not workers drawn from other jobs — the true opportunity cost of their
labor before the program was low.
...
Net social benefit attributable per hour worked ≈ £11.00 − £6.60 = £4.40/hour
  (the "extra" value created by moving genuinely idle labor into production,
   as distinct from the wage itself, which is largely a transfer)
```

This is why appraisals of employment programs in high-unemployment areas can show a positive net
social value even when the same program, run in a full-employment area where displaced labor
...
claims a carbon or social benefit from an IT change — a data-center consolidation claiming carbon
...
- **Confusing the shadow wage with a discount on the worker's actual pay.** The shadow wage rate
  adjusts the *appraisal's* valuation of the labor input, not the wage the worker is actually
...
- HM Treasury. "The Green Book: appraisal and evaluation in central government," Annex A (shadow
  price of labor, non-work time values).
