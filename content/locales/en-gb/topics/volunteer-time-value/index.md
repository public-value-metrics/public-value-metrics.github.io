...
Volunteer time value is the monetary estimate assigned to unpaid labour, most often used to state
a charity's true economic footprint — its accounts plus the labour it did not have to pay for — or
...
The reason this matters beyond accounting cosmetics: a programme that relies heavily on volunteer
labour can look dramatically cheaper on a pure cash [cost per outcome](../cost-per-outcome/) basis
than one relying on paid staff, even where the true resource cost — what it would cost to replace
that labour — is similar or higher. Funders and evaluators who ignore volunteer time value
...
labour — is £372,150, roughly 24% higher than the cash figure alone suggests. A cost-per-outcome
