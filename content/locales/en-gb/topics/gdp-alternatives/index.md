...
from current wellbeing rather than folded into one number. GDP alternatives operationalise that
...
consumption (as GDP does) and then adds non-market benefits GDP omits (household labour,
...
Centre Bhutan (<https://www.gnhcentre.bt/>), goes further still, replacing growth as the country's
...
**Region, GPI**: personal consumption is $50bn. Add estimated household and volunteer labour
...
- A KPI dashboard modelled only on throughput or spend (the GDP pattern) will systematically miss
...
- **Treating GPI as a precise national account** — unlike GDP, GPI has no single standardised
...
- **Assuming a GDP alternative replaces cost-benefit appraisal** — these are diagnostic,
  economy-wide indicators, not decision tools for a single programme; use
