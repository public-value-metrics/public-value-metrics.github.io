...
Impact evaluation asks whether a programme caused its intended outcomes. Process evaluation asks
whether the programme was actually delivered as designed — to whom, at what dose, and with what
...
programme's underlying theory was wrong, or whether a good theory was simply never properly
delivered.

## Why it matters

Government evaluations have repeatedly found no measurable effect from a programme while having no
...
process evaluation risks abandoning a genuinely sound programme design because it was delivered to
...
**Local authority (parenting programme)**: an impact evaluation using difference-in-differences
finds a +2 percentage point change in a child wellbeing measure — not statistically significant.
The process evaluation, run alongside it, finds the programme reached only 210 of the 500 families
...
consistent with an implementation failure, not evidence the programme model doesn't work; the
appropriate response is fixing the referral pathway that caused the 58% drop-off, not abandoning
the programme design.

**Charity (digital literacy programme)**: an impact evaluation finds a strong effect (+18 percentage
points on a digital confidence score), and a parallel process evaluation confirms 92% fidelity to
the planned curriculum across all 12 delivery sites. Combined, the funder can scale the programme
...
programme's delivery pipeline instead. Feeding fidelity and reach metrics to programme managers in
near-real time, rather than waiting for an end-of-grant evaluation, lets a broken referral pathway
get fixed mid-programme instead of being discovered only once the funding period has ended. See
...
[benefits realisation](../benefits-realisation/) for tracking delivery through to the outcomes
that were promised.
